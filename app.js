...
function openUrlOrAlert(url, msg) {
  if (!url) {
    alert(msg || "Falta el enlace en data.json");
    return;
  }
...
function renderEmociones() {
  ...
  root.querySelector("#acceptChallenge").onclick = () => {
    openUrlOrAlert(
      DATA.links?.challengeFormUrl,
      "Añade el enlace del reto en data.json → links.challengeFormUrl"
    );
  };

  root.querySelector("#openFeedback").onclick = () => {
    openUrlOrAlert(
      DATA.links?.feedbackFormUrl,
      "Añade el enlace de feedback en data.json → links.feedbackFormUrl"
    );
  };
...
function renderRelatos() {
  ...
  // botón añadir relato (simulación)
...
/* Modal detalle — disclaimer (sin contenido) */
    ...
    <div class="modal">
      ...
      <div class="modalBody">
        <div class="card" style="margin:0;">
          ...
          <div class="small" style="font-size:13px; line-height:1.55;">
            En esta demo no se muestra el contenido completo del testimonio original para proteger a las participantes.
